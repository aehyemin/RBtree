...
void delete_rbtree(rbtree *t) {
	if (t->root != t->nil) {
		delete_node(t, t->root);//루트노드부터 모든노드삭제
	}
...
void rbtree_insert_fixup(rbtree *t, node_t *z) {
	while (z->parent->color == RBTREE_RED) { //부모가 red
		
		if (z->parent == z->parent->parent->left) { //1자인상태
			node_t *right_uncle = z->parent->parent->right;
			...
			//case2. 할아버지,부모,자신 꺾인 상태
			//회전해서 부모,자신 펴준다. case3 상태가 됨
...
int order(const rbtree *t, node_t *node) {
